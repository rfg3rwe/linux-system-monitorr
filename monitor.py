...
class SysMonApp:
    ...
    def setup_style(self):
        ...
        style.configure('TButton', background='#333333', foreground='#ffffff', borderwidth=1, focuscolor='none', font=('Segoe UI', 9))
        style.map('TButton', background=[('active', '#444444')])
        style.configure('Exit.TButton', background='#333333', foreground='#F44336', borderwidth=1, focuscolor='none', font=('Segoe UI', 9, 'bold'))
        style.map('Exit.TButton', background=[('active', '#553333')])
        style.configure('TNotebook', background='#1e1e1e', borderwidth=0)
        style.configure('TNotebook.Tab', background='#2a2a2a', foreground='#aaaaaa', padding=[15, 8], font=('Segoe UI', 10))
    ...
    def setup_ui(self):
        ...
        tk.Label(header, text='LINUX SYSTEM MONITOR', font=('Segoe UI', 15, 'bold'), bg='#141414', fg='#e0e0e0').pack(pady=11)
        ...
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, padx=15, fill=tk.Y, pady=4)
        self.auto_refresh_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(toolbar, text='Auto-refresh', variable=self.auto_refresh_var, command=self.on_auto_refresh_toggle).pack(side=tk.LEFT, padx=3)
        tk.Label(toolbar, text=f'Interval: {INTERVAL}s', bg='#252525', fg='#888888', font=('Segoe UI', 9)).pack(side=tk.RIGHT, padx=10)
        ttk.Button(toolbar, text='Exit', command=self.on_close, style='Exit.TButton').pack(side=tk.RIGHT, padx=3)
        ...
        self.status_bar = tk.Label(self.root, text='Ready', bg='#141414', fg='#777777', anchor=tk.W, padx=12, pady=4, font=('Segoe UI', 8))
    ...
    def create_metric_card(self, parent, title, tag):
        ...
        tk.Label(header, text=title, font=('Segoe UI', 11, 'bold'), bg='#252525', fg='#cccccc').pack(side=tk.LEFT)
        value_label = tk.Label(header, text='0%', font=('Segoe UI', 13, 'bold'), bg='#252525', fg='#4CAF50')
    ...
    def setup_graphs_tab(self, parent):
        ...
        tk.Label(inner, text='CPU History', font=('Segoe UI', 10, 'bold'), bg='#252525', fg='#cccccc').pack(anchor=tk.W)
        self.cpu_graph = MiniGraph(inner, width=700, height=80, color='#4CAF50')
        self.cpu_graph.pack(fill=tk.X, pady=(4, 16))
        tk.Label(inner, text='RAM History', font=('Segoe UI', 10, 'bold'), bg='#252525', fg='#cccccc').pack(anchor=tk.W)
    ...
    def update_progress(self, canvas, value, max_width=None):
        ...
        canvas.create_rectangle(0, 0, fill_width, 6, fill=color, outline='')
    ...
    def update_display(self, data):
        ...
        self.cpu_value_label.config(text=f'{data["cpu"]:.1f}%', fg=get_color(data['cpu']))
        self.update_progress(self.ram_canvas, data['ram_percent'])
        self.ram_value_label.config(text=f'{data["ram_percent"]:.1f}%', fg=get_color(data['ram_percent']))
        self.update_progress(self.disk_canvas, data['disk_percent'])
        self.disk_value_label.config(text=f'{data["disk_percent"]:.1f}%', fg=get_color(data['disk_percent']))
        ...
        if disk_w > 4:
            ...
            self.disk_bar.create_rectangle(0, 0, fill_w, 8, fill=get_color(data['disk_percent']), outline='')
    ...
    def update_loop(self):
        if self.running and self.auto_refresh_var.get():
            data = self.stats.get_all()
            self.update_display(data)
            self.status_bar.config(text=f'Updated: {datetime.now().strftime("%H:%M:%S")} | CPU {data["cpu"]:.0f}% | RAM {data["ram_percent"]:.0f}%')
    ...
    def show_logs(self):
        ...
        text = scrolledtext.ScrolledText(log_window, bg='#141414', fg='#aaaaaa', font=('Cascadia Code', 10), insertbackground='#aaaaaa', relief=tk.FLAT, borderwidth=0, padx=12, pady=12)
